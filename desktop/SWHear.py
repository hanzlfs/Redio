...
class SWHear():
    ...
    def initiate(self):
        ...
        if not self.valid_test(self.device,self.rate):
            ...
            self.device=self.valid_input_devices()[0] #pick the first one
            self.rate=self.valid_low_rate(self.device)
        self.datax=np.arange(self.chunk*2)/float(self.rate)
...
if __name__=="__main__":
    ear=SWHear(updatesPerSecond=5) # optinoally set sample rate here
    ear.stream_start() #goes forever
    lastRead=ear.chunksRead
    while True:
        while lastRead==ear.chunksRead:
            time.sleep(.01)
        print(ear.chunksRead,len(ear.datax), len(ear.data))
        lastRead=ear.chunksRead
    print("DONE")
